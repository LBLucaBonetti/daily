...
class NoteControllerTests {
    ...
    void givenNoAuth_whenReadNotes_thenUnauthorized() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/notes").accept(MediaType.APPLICATION_JSON))
    ...
    void givenTwoNotes_whenReadNotes_thenOk() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/notes").with(jwt()).accept(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$.[*].uuid", containsInAnyOrder(n1dto.getUuid().toString(), n2dto.getUuid().toString())));
    }
    ...
    void givenWrongTextNote_whenCreateNote_thenBadRequest() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.post("/notes").with(jwt()).content(objectMapper.writeValueAsString(n4dto)).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
    ...
    void givenWrongTextNote_whenUpdateNote_thenBadRequest() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.put("/notes/{uuid}", uuid1).with(jwt()).content(objectMapper.writeValueAsString(n4dto)).accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest());
    }
    ...
    void givenNoNotes_whenUpdateNote_thenNotFound() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.put("/notes/{uuid}", uuid1).with(jwt()).content(objectMapper.writeValueAsString(n3dto)).accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound());
    }
